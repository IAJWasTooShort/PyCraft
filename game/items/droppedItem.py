...
class droppedItem:
    ...
    def update(self):
        ...
        for i in cpy:
                ...
                for ys in (1, 0, -1, -2):
                    for zs in (1, 0, -1):
                        if roundPos((pp[0] + xs, pp[1] + ys, pp[2] + zs)) == roundPos((x + kx, y + ky, z + kz)) and \
                                self.gl.player.hp > 0:
                            self.items.pop(i[0])
                            self.gl.itemsound.playPickUpSound()
                            self.gl.player.inventory.addItem(i[1][1])
                            br = True
                        if br:
                            break
                    if br:
            ...
            item = self.gl.item[i[1][1]]
            tex_coords = ('t2f', (0, 0, 1, 0, 1, 1, 0, 1))
            self.gl.stuffBatch.add(4, GL_QUADS, item[4], ('v3f', vertexes[0]),
                                   tex_coords)  # back
            if i[1][5]:
                self.gl.stuffBatch.add(4, GL_QUADS, item[5], ('v3f', vertexes[1]),
                                       tex_coords)  # front

                self.gl.stuffBatch.add(4, GL_QUADS, item[0], ('v3f', vertexes[2]),
                                       tex_coords)  # left
                self.gl.stuffBatch.add(4, GL_QUADS, item[1], ('v3f', vertexes[3]),
                                       tex_coords)  # right

                self.gl.stuffBatch.add(4, GL_QUADS, item[2], ('v3f', vertexes[4]),
                                       tex_coords)  # bottom
                self.gl.stuffBatch.add(4, GL_QUADS, item[3], ('v3f', vertexes[5]),
                                       tex_coords)  # top
